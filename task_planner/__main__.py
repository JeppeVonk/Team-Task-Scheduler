...
def main():
    ...
    data_loader.validate_columns(df_t, {"taak", "scope", "aantal"}, "taken.csv")
    data_loader.validate_columns(
        df_w,
        {"jaar", "maand", "dag", "club", "team", "isUit"},
        "wedstrijden.csv",
    )
    data_loader.validate_columns(df_s, {"naam", "displaynaam"}, "spelers.csv")
    data_loader.validate_columns(df_a, {"club", "afstand_km"}, "afstanden.csv")
